import fs from "fs";
import { runner } from "node-pg-migrate";
import { Paths } from "~/infra/paths";
import type { AuthConfig, DatabaseConfig } from "~/shared/config/Config";
import { UnauthorizedException } from "~/shared/errors/ApplicationErrors";
import type { DatabaseGateway } from "~/shared/gateway/DatabaseGateway";

const noop = () => {};
const noopLogger = { debug: noop, info: noop, warn: noop, error: noop };
const UNDEFINED_TABLE_ERROR_CODE = "42P01";

type PostgresErrorLike = {
  code?: string;
};

export class MigrationService {
  private database: DatabaseGateway;
  private databaseConfig: DatabaseConfig;
  private authConfig: AuthConfig;

  constructor(
    database: DatabaseGateway,
    databaseConfig: DatabaseConfig,
    authConfig: AuthConfig,
  ) {
    this.database = database;
    this.databaseConfig = databaseConfig;
    this.authConfig = authConfig;
  }

  async listPendingMigrations(): Promise<string[]> {
    const files = fs.readdirSync(Paths.migrationsDir());
    const allFiles = files
      .filter((f) => /^\d+_.+\.(ts|js)$/.test(f))
      .sort()
      .map((f) => f.replace(/\.(ts|js)$/, ""));
    let applied: string[] = [];
    try {
      const rows = await this.database.sql.unsafe<{ name: string }[]>(
        "SELECT name FROM pgmigrations ORDER BY run_on",
      );
      applied = rows.map((r) => r.name);
    } catch (error) {
      if ((error as PostgresErrorLike).code !== UNDEFINED_TABLE_ERROR_CODE) {
        throw error;
      }

      // pgmigrations table doesn't exist yet (e.g. after wipe) — all are pending
    }
    return allFiles.filter((f) => !applied.includes(f));
  }

  async runPendingMigrations(hashPassword?: string): Promise<string[]> {
    if (this.authConfig.hashPassword !== hashPassword) {
      throw new UnauthorizedException("Invalid password");
    }
    const result = await runner({
      databaseUrl: this.databaseConfig.connectionString,
      dir: Paths.migrationsDir(),
      direction: "up",
      migrationsTable: "pgmigrations",
      noLock: true,
      logger: noopLogger,
    });
    return result.map((m) => m.name);
  }

  async resetMigrations(hashPassword?: string): Promise<void> {
    if (this.authConfig.hashPassword !== hashPassword) {
      throw new UnauthorizedException("Invalid password");
    }
    let appliedCount = 0;
    try {
      const rows = await this.database.sql.unsafe<{ count: string }[]>(
        "SELECT COUNT(*)::text as count FROM pgmigrations",
      );
      appliedCount = parseInt(rows[0].count, 10);
    } catch (error) {
      if ((error as PostgresErrorLike).code !== UNDEFINED_TABLE_ERROR_CODE) {
        throw error;
      }

      return;
    }
    if (appliedCount === 0) return;
    await runner({
      databaseUrl: this.databaseConfig.connectionString,
      dir: Paths.migrationsDir(),
      direction: "down",
      migrationsTable: "pgmigrations",
      count: appliedCount,
      noLock: true,
      logger: noopLogger,
    });
  }
}
