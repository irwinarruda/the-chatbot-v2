...
import { buildScopes } from "~/server/resources/GoogleAuthScopes";
import type {
  GoogleTokens,
  GoogleUserInfo,
  IGoogleAuthGateway,
...
export class GoogleAuthGateway implements IGoogleAuthGateway {
  ...
  createAuthorizationCodeUrl(
    ...
    const oauth2Client =
      redirectTarget === "web" ? this.webOAuth2Client : this.oauth2Client;
  ...
  async exchangeCodeForTokens(code: string): Promise<GoogleTokens> {
    const clients = [this.oauth2Client, this.webOAuth2Client];
    ...
    throw lastError instanceof Error
      ? lastError
      : new Error("Failed to obtain required tokens from Google");
  }
