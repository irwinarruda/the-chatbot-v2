export class WhatsAppTextChunker {
  ...
  private static findSplitIndex(text: string): number {
    ...
    const sentenceEnd = WhatsAppTextChunker.findLastSentenceEnd(searchRegion);
  ...
  static chunk(text: string): string[] {
    ...
    while (remaining.length > WhatsAppTextChunker.MaxChunkSize) {
      const splitIndex = WhatsAppTextChunker.findSplitIndex(remaining);
